...
def compare_beacons(all_beacons, scanner_2):
    ...
    for diff, overlap in s1_overlaps.items():
        if not best_diff or len(overlap) > max_overlaps:
            ...
            best_overlaps = overlap

    return max_overlaps, best_diff, best_overlaps
...
def get_overlaps(all_beacons, initial_scanner):
    ...
    for pos_neg in get_pos_negs():
        for swap in get_swaps():
            ...
            overlap, diff, best_overlaps = compare_beacons(all_beacons, scanner)

            if overlap > max_overlap:
                max_overlap = overlap
                matched_scanner = scanner
                best_diff = diff
                best_best_overlap = best_overlaps

    return max_overlap, matched_scanner, best_diff, best_best_overlap
...
while scanners:
    ...
    num_overlaps, matched_scanner, diff, bbo = get_overlaps(all_beacons, scanner)
